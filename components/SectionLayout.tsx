    ...
    <main className="relative min-h-screen overflow-hidden bg-[#03030b] text-white">
      ...
      <aside className="fixed left-0 top-0 flex h-screen w-44 flex-col items-center justify-start border-r border-violet-200/10 bg-[#050510]/78 px-4 pt-8 shadow-[20px_0_80px_rgba(0,0,0,0.28)] backdrop-blur-2xl">
        <div className="flex flex-col items-center gap-6 w-full">
          ...
          <div className="flex flex-col items-center gap-2 text-xs font-medium uppercase tracking-[0.12em] text-violet-100/65">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`flex flex-col items-center gap-2 rounded-3xl px-3 py-2 transition ${active ? 'bg-violet-500/12 text-white shadow-[0_0_34px_rgba(139,92,246,0.2)]' : 'hover:text-white'}`}
        ...
        <div className="w-full max-w-[1180px] text-left">
          <div className="mb-10 flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <span className="rounded-full bg-violet-500/12 px-3 py-1 text-sm font-medium text-violet-100">Prototype</span>
            </div>
            <p className="max-w-2xl text-lg text-[#B7C8DA]">{subtitle}</p>
            <Link href="/" className="text-sm text-violet-200 transition hover:text-white">
              Back to home
            </Link>
          </div>
          {children}
